...
<script>
import calendar from './calendarJs.js'
export default {
    props: {
        // 多选模式
        multi: {
...
//                     console.log("选中日期",this.month)
                    let b = [this.rangeBegin[0],this.rangeBegin[1]+1,this.rangeBegin[2]];
                    let e = [this.rangeEnd[0],this.rangeEnd[1]+1,this.rangeEnd[2]];
                    this.$emit('select',begin,end,[b,e])
                }
                this.render(this.year, this.month)
            }else if (this.multi) {
                // 如果已经选过则过滤掉
