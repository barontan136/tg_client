<template>
  <div class="bargainlist_box" style="width:100%" v-wechat-title="$route.meta.title">
    ...
    <div slot="default" class="vux_list">
      ...
      <!-- 用户参团信息 -->
      <div class="tg_user_info_index"  v-if="orderlist.length>0">
          <div class="marquee">
              <marquee behavior="scroll" direction="up" scrollamount="10">
                  <ul>
                      <li v-for="(item,kone) in orderlist" :key='kone'>
                          ...
                          <span v-if="item.order_type == 1" class="align-middle-li">刚刚购买了</span>
    ...
    <div class="container_body">
        <div class="tg_act_info tg-content" v-if="loading_show === true">
            <div class="price">
                <span class="t1">浏览人次:</span>
                <span class="t2">{{baseinfo.view_num}}</span>
            </div>
            <div class="kj_time">
                <p>活动倒计时</p>
                <!--<p>{{baseinfo.end_date}}</p>-->
                <p>
                <span>{{act_last_time[0]}}</span><em>天</em>
                <span>{{act_last_time[1]}}</span><em>时</em>
                <span>{{act_last_time[2]}}</span><em>分</em>
                <span class="second">{{act_last_time[3]}}</span><em>秒</em>
                </p>
            </div>
        </div>
        <!-- end 活动时间 -->
        <div class="top_images tg-content" v-if="loading_show === true">
            <div class="tit">参与人数:{{baseinfo.jion_num}}人</div>
            <div class="bd" v-if="orderlist.length>0">
                <ul id="scrolluser">
                    <li v-for="(item,kone) in orderlist" :key="kone">
        ...
        <!-- end 参与人数 -->
        <div class="top_images tg-content" v-if="loading_show === true">
            <div class="tit">购买记录:{{baseinfo.group_num}}人</div>
            <div class="list">
                <div class="items" v-if="orderlist.length>0" v-for="(item,kone) in orderlist" :key="kone">
                    <div class="tg_buy_info" v-if="loading_show === true">
                        <div class="info">
                            <img class="buy_images" v-bind:src="item.head_img">
                            <span>{{item.nick_name}}</span>
                            <span>{{item.mobile}}</span>
                        </div>
                        <div class="time">
                            <p>已支付：¥{{item.pay_amount}}</p>
                            <p>{{item.pay_time}}</p>
                        </div>
                    </div>
                </div>
    ...
    <van-popup v-model="show_input" position="bottom" class="buy-info-box">
      ...
      <van-popup v-model="kf_show" style="width: 70%;border-radius:8px;">
          <div class="kf_info">拨打客服电话：<span>{{baseinfo.activity_telephone}}</span><br><span>长按添加客服</span></div>
          <div><img style="width:100%" :src="dcrc_icon"></div>
...
export default {
  ...
  data() {
    return {
      ...
      group_color_css: "",
      brokerage_color_css: "",
      group_type: "1", //1-直接购买 2-参团购买
      group_type_t: "1", //1-直接购买 2-参团购买 3-参加团
      name: "", //姓名
      telephone: "", //联系电话
      disabled: false,
